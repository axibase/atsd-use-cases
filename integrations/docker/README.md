...
The synthetic `on-error` payload is similar to the native `on-success` webhook above except `repository.status` is set to **Failed** (in case of build failure) or **Queued** (in case of hanging build). An extra `build_history` object is injected for additional detail.
